...
def build_model_cfg(cfg: dict) -> dict:
    return deep_update(cfg.get("model", {}), cfg.get("rpfn", {}))
...
def main() -> None:
    ...
    parser.add_argument("--reset-input", default="gt")
    ...
    model = RPFN(build_model_cfg(cfg)).to(device)
    ...
    for epoch in range(epochs):
        ...
        if val_metrics["loss"] <= best:
            ...
            save_checkpoint(out_dir / "checkpoints" / "best.pt", model, build_model_cfg(cfg), {"val_loss": best, "reset_input": args.reset_input})
