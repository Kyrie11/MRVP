...
def main() -> None:
    ...
    load_model(msrt, args.msrt, args.device, strict=False)
    load_model(rpn, args.rpn, args.device, strict=False)
    table = load_calibration_table(args.calibration or None)
    sel = select_action_with_models(root_batch, root_rows, msrt, rpn, table, num_samples=args.num_samples, beta=args.beta, device=args.device)
